...
def menu_item_photo_upload_to(instance, filename):
    """
    Store menu item images in MEDIA_ROOT/item-image/ using the item name.
    A short UUID suffix avoids collisions when two items share the same name.
    """
    extension = filename.rsplit('.', 1)[-1].lower() if '.' in filename else 'jpg'
    item_name = slugify(instance.name) or 'menu-item'
    return f"item-image/{item_name}-{uuid.uuid4().hex[:8]}.{extension}"
...
class CanteenMenuItem(models.Model):
    ...
    photo = models.ImageField(
        upload_to=menu_item_photo_upload_to,
        blank=True,
        null=True,
    ...
    discounted_price = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text='Optional discounted price visible only to eligible employee categories.',
    )
